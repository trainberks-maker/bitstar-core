...
Options:
  -m, --multiprocess     Run multiprocess binaries bitstar-node, bitstar-gui.
  -M, --monolithic       Run monolithic binaries bitstard, bitstar-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'bitstar-qt [ARGS]' or 'bitstar-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'bitstard [ARGS]' or 'bitstar-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'bitstar-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'bitstar-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'bitstar-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_bitstar [ARGS]'.
  chainstate [ARGS] Run BitStar kernel chainstate util, equivalent to running 'bitstar-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_bitstar [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_bitstar-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "bitstar-gui" : "bitstar-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "bitstar-node" : "bitstard");
        } else if (cmd.command == "rpc") {
            args.emplace_back("bitstar-cli");
            // Since "bitstar rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("bitstar-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("bitstar-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_bitstar");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("bitstar-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_bitstar");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_bitstar-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("bitstar-util");
        } else {
