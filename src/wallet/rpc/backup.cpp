...
RPCMethod removeprunedfunds()
{
    return RPCMethod{
        "removeprunedfunds",
        "(DEPRECATED) This feature will be removed in the next major release. Start bitstard with the `-deprecatedrpc=removeprunedfunds` option in order to use this.\n"
        ...
        [](const RPCMethod& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!pwallet->chain().rpcEnableDeprecated("removeprunedfunds")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "DEPRECATION WARNING: This feature will be removed in the next major release. Start bitstard with the `-deprecatedrpc=removeprunedfunds` option in order to use this.");
    }
                ...
                if (scanned_time <= GetImportTimestamp(request, now) || results.at(i).exists("error")) {
                    ...
                    if (pwallet->chain().havePruned()) {
                        error_msg += strprintf(" This error could be caused by pruning or data corruption "
                                "(see bitstard log for details) and could be dealt with by downloading and "
