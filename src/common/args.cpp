...
const char * const BITCOIN_CONF_FILENAME = "bitstar.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\BitStar
    //   new: C:\Users\Username\AppData\Local\BitStar
    // macOS: ~/Library/Application Support/BitStar
    // Unix-like: ~/.bitstar
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "BitStar";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "BitStar";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/BitStar";
#else
    // Unix-like
