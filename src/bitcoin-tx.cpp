...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " bitstar-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The bitstar-tx tool is used for creating and modifying BitStar transactions.\n\n"
                "bitstar-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded BitStar transaction, or with \"-create [commands]\" to create a hex-encoded BitStar transaction.\n"
                "\n"
                "Usage: bitstar-tx [options] <hex-tx> [commands]\n"
                "or:    bitstar-tx [options] -create [commands]\n"
                "\n";
